...
<div class="layui-fluid">
    <div class="layui-card">
        ...
        <div class="layui-card-body">
            ...
            <table id="layTable" class="layui-table" lay-data="{url:'/admin/sysuser/sysUserList', page:true, toolbar:'#toolbarDemo'}" lay-filter="layTable">
                <thead>
                <tr>
                    ...
                    <th lay-data="{field:'wxNumber',width:120}">微信号码</th>
...
<script type="text/javascript">
    layui.use(['form','table','laypage'], function () {
        ...
        table.on('tool(layTable)', function(obj){
            ...
            let id = data.id;
            if(obj.event === 'edit'){
                $.getJSON("sysuser/get/" + id, {}, function (data) {
                    if (!data.success) {
                        layer.alert("管理员不存在");
                        return false;
                    }
            ...
            if(obj.event === 'unlock'){
                layer.confirm("确定要对管理员【"+data.realname+"】进行登录解锁？",function () {
                    layer.load(1);
                    $.ajax({
                        url:"/admin/sysuser/login-unlock/" + id,
                        type:"PUT",
            ...
            if(obj.event === 'reset'){
                layer.confirm("确定要对管理员【"+data.realname+"】进行密码重置？",function () {
                    layer.load(1);
                    $.ajax({
                        url:"/admin/sysuser/reset-password/" + id,
                        type:"PUT",
                        success:function (data) {
                            if(!data.success){
                                layer.closeAll();
            ...
            if(obj.event === 'del'){
                layer.confirm("确定要删除管理员【"+data.realname+"】？",function () {
                    layer.load(1);
                    $.ajax({
        ...
        form.on('switch(laySwitch)', function (data) {
            ...
            layer.load(1);
            $.ajax({
                url: "/admin/sysuser/modify-status/" + sysUserId,
                type: "PUT",
                success: function (data) {
                    layer.closeAll();
                    if (!data.success) {
